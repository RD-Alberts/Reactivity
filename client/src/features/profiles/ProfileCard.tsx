...
export const ProfileCard = ({ profile }: Props) => {
  ...
  return (
    <Link to={`/profiles/${profile.id}`} style={{ textDecoration: "none" }}>
      <Card
        sx={{ borderRadius: 3, p: 3, textDecoration: "none", maxWidth: 250 }}
        elevation={4}
      >
        <CardMedia
          component="img"
        ...
        <CardContent>
            <Box display='flex' flexDirection='column' gap={1}>
                ...
                {profile.bio && (
                  <Typography variant="body2" sx={{textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'no-wrap'}}>{profile.bio}</Typography>
                )}
